...
import { getRendererComponent } from '../../utils/layoutItemRendererUtils'; // Assuming you might centralize this

// Conditional console logging for debugging
...
const VenueLayoutPreview = ({ layoutData, openAlert, isZenMode }) => {
    ...
    const tablesForQr = useMemo(
        ...
        ).sort((a, b) => (a.number || 0) - (b.number || 0)), // Sort tables by number
        [designItems]
    );
    ...
    const previewGridStyle = useMemo(() => ({
        ...
        backgroundColor: 'var(--color-background-grid-preview, hsl(220 10% 98%))', // Lighter slate-50
        border: '1px solid var(--color-border-grid-preview, hsl(220 10% 92%))',    // Lighter slate-200
        borderRadius: '0.5rem', // Slightly larger radius
        boxShadow: '0 1px 3px 0 rgba(0,0,0,0.03), 0 1px 2px -1px rgba(0,0,0,0.03)', // Softer shadow
        position: 'relative',
        margin: 'auto',
    }), [totalMinorRows, totalMinorCols, minorCellSizeRem, rows, cols]);
    ...
    return (
        <div className={`flex h-full ${isZenMode ? 'flex-col' : 'flex-row'} overflow-hidden `}>
            <div className={`flex-1 flex items-center justify-center p-4 md:p-6 overflow-auto ${isZenMode ? 'h-full' : 'h-full'}`}>
                <div style={previewGridStyle}>
                    ...
                    {designItems.map(item => {
                        ...
                        const Renderer = getRendererComponent(item.itemType, item.decorType); // Use centralized getter
                        ...
                        return (
                            <div
                                ...
                                className={`transition-all duration-150 ease-in-out 
                                    ${isClickableTable ? 'group hover:ring-2 hover:ring-rose-500 dark:hover:ring-rose-400 hover:shadow-md rounded-lg' : ''}`
                                }
                            ...
                            >
                                <Renderer item={item} itemRotation={item.rotation || 0} isSelected={false} isPreviewMode={true} zoomLevel={1} />
                            </div>
                        );
                    })}
                </div>
            ...
            {!isZenMode && (
                <aside
                    className="
                        w-72 lg:w-80 xl:w-[22rem] h-full flex flex-col 
                ...
                >
                                ...
                                >
                                    <div className="flex justify-between items-center mb-1.5">
                                        ...
                                        <button
                                            ...
                                            className="p-1 rounded-md text-neutral-500 dark:text-neutral-400 hover:bg-neutral-100 dark:hover:bg-neutral-600 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                    ...
                    {tablesForQr.length > 0 && (
                        <div className="p-3 border-t border-neutral-200/80 dark:border-neutral-700/60 shrink-0">
                            ...
                            >
                                <Icon name="file_download" className="w-4 h-4 mr-1.5" />
                                Download All QRs
                            </button>
                        </div>
                    )}

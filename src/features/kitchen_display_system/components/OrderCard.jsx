...
const OrderCard = ({ order, onUpdateStatus, onSelectOrder }) => {
    const timeAgo = order.order_timestamp ? formatDistanceToNowStrict(parseISO(order.order_timestamp)) : (sl.timeAgoNA);
    ...
    const getNextStatusAndAction = () => {
        switch (order.status) {
            ...
            case 'PREPARING': return { nextStatus: 'SERVED', actionText: sl.actionMarkServed, icon: "room_service" };
    ...
    const cardStyles = () => {
        switch (order.status) {
            case 'PENDING_CONFIRMATION': return { bg: 'bg-amber-50 dark:bg-amber-900/40', border: 'border-amber-400 dark:border-amber-500', accentText: 'text-amber-600 dark:text-amber-400', actionButton: 'bg-amber-500 hover:bg-amber-600' };
            ...
            case 'PREPARING': return { bg: 'bg-purple-50 dark:bg-purple-900/40', border: 'border-purple-400 dark:border-purple-500', accentText: 'text-purple-600 dark:text-purple-400', actionButton: 'bg-purple-500 hover:bg-purple-600' };
            case 'READY_FOR_PICKUP':
            case 'SERVED': return { bg: 'bg-emerald-50 dark:bg-emerald-900/40', border: 'border-emerald-400 dark:border-emerald-500', accentText: 'text-emerald-600 dark:text-emerald-400', actionButton: 'bg-emerald-500' };
    ...
    const orderIdentifier = order.table_number
        ...
        : (order.pickup_code ? i18n.t(sl.pickupIdentifier, { pickupCode: order.pickup_code }) : (sl.defaultIdentifier));

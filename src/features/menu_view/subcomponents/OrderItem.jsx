...
function OrderItem({
    ...
    // MODIFIED: Comprehensive ARIA Label with formatCurrency
    const ariaLabel = `${itemName}, quantity ${item.quantity}, final price ${formatCurrency(displayLineTotal)}${showDiscountedPrice ? ` (discounted from ${formatCurrency(originalLineTotal)}, ${discountDescriptionToShow || 'special discount'})` : ''}`.trim().replace(/\s+/g, ' ');
    ...
    return (
        ...
        >
            ...
            <div className="flex-grow min-w-0">
                ...
                {/* MODIFIED: Format unit price */}
                <p className={`${ITEM_PRICE_UNIT_TEXT_SIZE} ${NEUTRAL_TEXT_SECONDARY} mt-1`}>{formatCurrency(itemBasePricePerUnit)} / unit</p>
            ...
            <div className="flex flex-col items-end space-y-2 ml-2 flex-shrink-0 w-[8.5rem] text-right">
                <div className="flex items-center space-x-2">
                    <motion.button onClick={handleDecrement} disabled={isPreviewMode} className={`disabled:opacity-50 disabled:cursor-not-allowed ${STEPPER_BUTTON_BG} ${STEPPER_BUTTON_TEXT} ${STEPPER_BUTTON_SIZE} ${STEPPER_BUTTON_RADIUS} flex items-center justify-center transition-colors ${STEPPER_BUTTON_BG_HOVER} focus:outline-none ${STEPPER_RING_FOCUS}`} whileTap={{ scale: 0.9 }} aria-label={`Decrease quantity of ${itemName}`}>
                    ...
                    <motion.button onClick={handleIncrement} disabled={isPreviewMode} className={`disabled:opacity-50 disabled:cursor-not-allowed ${STEPPER_BUTTON_BG} ${STEPPER_BUTTON_TEXT} ${STEPPER_BUTTON_SIZE} ${STEPPER_BUTTON_RADIUS} flex items-center justify-center transition-colors ${STEPPER_BUTTON_BG_HOVER} focus:outline-none ${STEPPER_RING_FOCUS}`} whileTap={{ scale: 0.9 }} aria-label={`Increase quantity of ${itemName}`}>
                        <Icon name="add" className={STEPPER_BUTTON_ICON_SIZE} style={{ fontSize: '1rem' }} />
                    </motion.button>
                </div>
                <div>
                    {/* MODIFIED: Format original and final line totals */}
                    {showDiscountedPrice && <del className={`${ITEM_OPTIONS_TEXT_SIZE} ${NEUTRAL_TEXT_MUTED}`} aria-label={`Original price ${formatCurrency(originalLineTotal)}`}>{formatCurrency(originalLineTotal)}</del>}
                    <p className={`${ITEM_TOTAL_PRICE_TEXT_SIZE} font-bold ${showDiscountedPrice ? DISCOUNT_PRICE_TEXT : NEUTRAL_TEXT_PRIMARY}`} aria-label={`Current total price ${formatCurrency(displayLineTotal)}`}>{formatCurrency(displayLineTotal)}</p>
                </div>
            </div>
        </motion.div>
    );

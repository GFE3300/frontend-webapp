...
export default function ProductOptionsPopup({ isOpen, onClose, product, onConfirmWithOptions }) {
        ...
        product.editable_attribute_groups.forEach(group => {
            if (group.is_required) {
                ...
                if (group.type === SINGLE_SELECT && (selection === null || selection === undefined || selection === '')) {
                    ...
                    // For multi-select, check if min_selectable is met, if such a property exists.
                    // Assuming for now, required multi-select needs at least one.
    ...
    const handleConfirm = () => {
        ...
        const payloadSelectedOptions = detailedSelectedOptions.map(opt => ({
            ...
            optionId: opt.id, // This is the original option ID from the backend
            optionName: opt.name,
        ...
        // Parent (AdminMenuPreviewPage) will call onClose via setIsOptionsPopupOpen(false)
    };

    if (!product) return null;

    const popupWidth = "w-[90vw] max-w-md"; // Consistent with Userpage styling

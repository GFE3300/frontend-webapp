...
// Maps backend statuses to UI themes and text from script lines.
const statusConfig = {
    PENDING_CONFIRMATION: {
        bgColor: "bg-gradient-to-br from-amber-400 to-amber-600",
        textColor: "text-white",
        icon: "hourglass_top",
        title: sl.orderTracker.pendingConfirmationTitle || "Waiting for Confirmation",
        message: sl.orderTracker.pendingConfirmationMessage || "The kitchen will confirm your order shortly."
    },
    CONFIRMED: {
        bgColor: "bg-gradient-to-br from-sky-500 to-sky-700",
        textColor: "text-white",
        icon: "thumb_up",
        title: sl.orderTracker.confirmedTitle || "Order Confirmed!",
        message: sl.orderTracker.confirmedMessage || "We've received your order and will start preparing it soon."
    },
    PREPARING: {
        bgColor: "bg-gradient-to-br from-indigo-500 to-indigo-700",
        textColor: "text-white",
        icon: "soup_kitchen",
        title: sl.orderTracker.preparingTitle || "Your Order is Being Prepared",
        message: sl.orderTracker.preparingMessage || "Our chefs are working their magic. It won't be long!"
    },
    SERVED: {
        bgColor: "bg-gradient-to-br from-lime-500 to-lime-700",
        textColor: "text-white",
        icon: "room_service",
        title: sl.orderTracker.servedTitle || "Your Order is on its Way!",
        message: sl.orderTracker.servedMessage || "Look out for our staff bringing your items."
    },
    READY_FOR_PICKUP: {
        bgColor: "bg-gradient-to-br from-teal-500 to-teal-700",
        textColor: "text-white",
        icon: "local_mall",
        title: sl.orderTracker.readyForPickupTitle || "Ready for Pickup!",
        message: sl.orderTracker.readyForPickupMessage || "Please show your pickup code at the counter."
    },
    COMPLETED: {
        bgColor: "bg-gradient-to-br from-green-500 to-green-700",
        textColor: "text-white",
        icon: "check_circle",
        title: sl.orderTracker.completedTitle || "Order Complete!",
        message: sl.orderTracker.completedMessage || "Thank you for your order. Enjoy!"
    },
    CANCELLED_BY_BUSINESS: {
        bgColor: "bg-gradient-to-br from-slate-600 to-slate-800",
        textColor: "text-white",
        icon: "cancel",
        title: sl.orderTracker.cancelledByBusinessTitle || "Order Cancelled",
        message: sl.orderTracker.cancelledByBusinessMessage || "There was an issue with the order. Please see staff for details."
    },
    CANCELLED_BY_CUSTOMER: {
        bgColor: "bg-gradient-to-br from-slate-600 to-slate-800",
        textColor: "text-white",
        icon: "cancel",
        title: sl.orderTracker.cancelledByCustomerTitle || "Order Cancelled",
        message: sl.orderTracker.cancelledByCustomerMessage || "Your order has been successfully cancelled."
    },
    default: {
        bgColor: "bg-gradient-to-br from-slate-500 to-slate-700",
        textColor: "text-white",
        icon: "help_outline",
        title: sl.orderTracker.defaultTitle || "Order Status Unknown",
        message: sl.orderTracker.defaultMessage || "We're currently updating the order status."
    }
};
    ...
    if (isLoading && !order) {
        return (
            <div className="fixed inset-0 bg-slate-100 dark:bg-slate-900 flex items-center justify-center z-[200]">
                <Spinner size="xl" message={sl.orderTracker.loadingMessage || "Loading your order status..."} />
            </div>
        );
    }
    ...
    if (isError) {
        return (
            <div className="fixed inset-0 bg-red-100 dark:bg-red-900/50 flex flex-col items-center justify-center p-8 text-center z-[200]">
                ...
                <h2 className="text-2xl font-bold text-red-700 dark:text-red-200 font-montserrat">{sl.orderTracker.errorTitle || "Could Not Load Order"}</h2>
                <p className="text-red-600 dark:text-red-300 mt-2 max-w-md">{error?.message || (sl.orderTracker.errorMessageDefault || "An unknown error occurred. Please show this screen to a staff member.")}</p>
                <button onClick={onReturnToMenu} className="mt-8 px-6 py-2 bg-red-600 text-white font-semibold rounded-full shadow-lg hover:bg-red-700 transition-colors">
                    {sl.orderTracker.backToMenuButton || "Back to Menu"}
                </button>
            </div>
        );
    }
            ...
            >
                ...
                {order?.pickup_code && (
                    <motion.div className="mt-8" initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1, transition: { delay: 0.6, duration: 0.5 } }}>
                        <p className="text-sm font-semibold opacity-80 uppercase tracking-widest">{sl.orderTracker.pickupCodeLabel || "Pickup Code"}</p>
                ...
                >
                    <p>{sl.orderTracker.timerLabel || "Time in current state: "}<Timer startTime={order.last_updated_timestamp} /></p>
                </motion.div>
                ...
                >
                    <button onClick={onReturnToMenu} className="px-5 py-2.5 bg-white/20 dark:bg-black/20 backdrop-blur-sm rounded-full text-sm font-semibold hover:bg-white/30 dark:hover:bg-black/30 transition-colors">
                        {sl.orderTracker.placeAnotherOrderButton || "Place Another Order"}
                    </button>
                </motion.div>

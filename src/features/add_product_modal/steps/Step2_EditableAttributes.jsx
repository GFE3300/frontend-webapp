...
const Step2_EditableAttributes_Actual = memo(({ formData, updateField, errors }) => {
    ...
    const sl = scriptLines.step2EditableAttributes; // MODIFIED: Alias for shorter access

    const handleAddGroup = () => {
        const newGroup = {
            id: generateId('attr_group_'),
            name: '',
            type: 'single_select',
            isRequired: false,
        ...
        <div className="flex flex-col gap-6 sm:gap-8 py-2">
            ...
            {attributes.length === 0 && !arrayLevelError && (
                ...
                >
                    ...
                    <p className="text-sm sm:text-md font-medium text-neutral-700 dark:text-neutral-200 mb-1 sm:mb-2">{sl.emptyStateTitle}</p> {/* MODIFIED */}
                    <p className="text-xs sm:text-sm text-neutral-500 dark:text-neutral-400">
                        {sl.emptyStateDescription} {/* MODIFIED */}
                    </p>
                </motion.div>
            )}
